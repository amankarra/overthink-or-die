...
export class BarrelSideScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.lampGlowGraphics = this.add.graphics().setDepth(11);
  ...
  private renderLampGlow(time: number): void {
    ...
    if (this.barrelMan.isRolling || this.barrelMan.isPlant) {
      return;
    }

    const lampX = Math.round(this.barrelMan.x + 7);
    const lampY = Math.round(this.barrelMan.y - 10);
